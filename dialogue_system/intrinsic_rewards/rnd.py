...
from src.dialogue_system.intrinsic_rewards.model import RNDNetwork
...
class RND(IntrinsicReward):
    ...
    def __init__(self, observation_size, action_size,**kwargs):
        ...
        """
        super(RND, self).__init__(observation_size, action_size,)

        self.model_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.max_grad_norm = .5
        self.lr = 2e-6
        self.intrinsic_reward_coef = 1.2
        self.rnd_loss_coef = 0.1
        # create models
        self.predictor_model = RNDNetwork(observation_size).to(self.model_device)
        self.target_model = RNDNetwork(observation_size).to(self.model_device)
