...
class User(object):
    ...
    def initialize(self, dataset, goal_index=None):
        ...
        # 不能发现自己隐含的一些症状。
        goal = self.goal["goal"]
        self.goal['disease_id'] = self.disease2id[self.goal['disease_tag']]
    ...
    def next(self, agent_action, turn):
        ...
        if agent_action == -1:
            return -1, 0, self.episode_over, dialogue_configuration.DIALOGUE_STATUS_NOT_COME_YET
        ...
        # if reward not in self.all_reward:
