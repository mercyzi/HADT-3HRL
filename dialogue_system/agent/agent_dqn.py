...
class AgentDQN(Agent):
    def __init__(self, action_set, slot_set, disease_symptom, parameter, disease_as_action=True):
        ...
        if self.symptom_dist_as_input is True and self.agent_id.lower() == 'agenthrl':
            ...
            temp_slot_set = copy.deepcopy(slot_set)
            if 'disease' in temp_slot_set.keys():
                temp_slot_set.pop('disease')
    ...
    def next(self, state, turn, greedy_strategy, **kwargs):
        ...
        if greedy_strategy is True:
            ...
            if greedy < self.parameter.get("epsilon"):
                for iter in range(len(self.action_space)):
                    ...
                    action = self.id2symptom[action_index]
                    # print(iter)
                    # print(action_index)
                    # print(action)
                    if action not in current_slots.keys():
                        break
            else:
                symptom_list = self.dqn.predict(Xs=[state_rep])[2]
                for iter in symptom_list:
                    action = self.id2symptom[iter]
                    action_index = iter
                    if action not in current_slots.keys():
                        break
        ...
        action_1 = self.id2symptom[action_index]
        if list(agent_action["request_slots"].keys())[0] in current_slots.keys():
            print('greedy_strategy',greedy_strategy)
            print('action_index',action_index)
            print(current_slots.keys())
            print(agent_action)
            print(action_1)
            assert 0
    ...
    def get_q_values(self, state, **kwargs):
        ...
        #    state_rep = np.concatenate((state_rep, goal), axis=0)
        #print(len(state_rep))
        Q_values, max_index, pre_sort = self.dqn.predict(Xs=[state_rep])
