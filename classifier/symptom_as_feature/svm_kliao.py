...
os.chdir(os.getcwd().replace('\\resource\\tagger2', ''))
...
def svm_model(dataset, min_count, target, svm_c, epoch):
    '''
    min_count:only slot with frequency bigger than min_count will be kept
    svm_c:the tunning parameter of svm
    epoch: the times of k-fold and then take the average
    '''
    index_len = len(dataset)
...
score_tot_exp = svm_model(dataset=slots_exp, min_count=0, target=disease_y, svm_c=0.3, epoch=10)
score_tot_all = svm_model(dataset=slots_all, min_count=0, target=disease_y, svm_c=0.3, epoch=10)
print("score of explicit is %f" % score_tot_exp)
print("score of explicit and implicit is %f" % score_tot_all)
'''
